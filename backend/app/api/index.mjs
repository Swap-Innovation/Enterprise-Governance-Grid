...
const DEMO_PARAM_DEFAULTS = { productId: 'dp-customer-360', natco: 'natco-de' }
function withDemoParams(params = {}) {
  return { ...DEMO_PARAM_DEFAULTS, ...(params && typeof params === 'object' ? params : {}) }
}
...
async function runView(viewId, queryParams) {
  ...
  if (viewId === 'natco-stack' && queryParams.get('natco')) params.natco = queryParams.get('natco')
  const data = await executeCypher(view.cypher, params, { compact: true, mode: 'graph' })
  return { ...data, view: viewId, title: view.title, description: view.description }
}
...
async function runCatalogQuery(queryId, options = {}) {
  ...
  return { ...data, queryId: q.id, code: q.code, title: q.title, description: q.description, sourceFile: q.sourceFile, group: q.group }
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && reqPath === '/api/kg/views') return sendJson(res, 200, { views: VIEW_CATALOG })
    if (req.method === 'GET' && reqPath === '/api/kg/queries') {
    ...
    const queryMatch = reqPath.match(/^\/api\/kg\/queries\/([^/]+)$/)
    if (req.method === 'GET' && queryMatch) {
      const q = QUERY_BY_ID.get(decodeURIComponent(queryMatch[1]))
      return q ? sendJson(res, 200, q) : sendJson(res, 404, { error: 'Unknown query' })
    }
    if (req.method === 'POST' && reqPath === '/api/kg/queries/run') {
      const body = await readBody(req)
      if (!body.queryId) return sendJson(res, 400, { error: 'queryId required' })
      const data = await runCatalogQuery(body.queryId, { mode: body.mode, compact: body.compact, params: body.params })
      return sendJson(res, 200, data)
    }
    if (req.method === 'POST' && reqPath === '/api/kg/run') {
      const body = await readBody(req)
      const data = await executeCypher(body.cypher, body.params ?? {}, { mode: body.mode ?? 'auto', compact: body.compact !== false })
      return sendJson(res, 200, data)
    }
    const viewMatch = reqPath.match(/^\/api\/kg\/views\/([^/]+)$/)
    ...
    return sendJson(res, err.status ?? 500, { error: err.message ?? 'Server error', source: 'neo4j' })
  }
})
